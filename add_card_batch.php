...
function addBatchCardsF($conn, $deck_id, $front, $back, $form_cardtype, $form_difficulty, $form_tags) {
    $stmt_insert = $conn->prepare("INSERT INTO cards (deck_id, question, answer, card_type, difficulty) VALUES (?,?,?,?,?)");
    $stmt_insert->bind_param("issss", $deck_id, $front, $back, $form_cardtype, $form_difficulty);

    if ($stmt_insert->execute()) {
        $new_card_id = $conn->insert_id; // FUCKING NEW CARD ID!!!
...
foreach ($lines as $line) {
    ...
    if (count($card_parts) === 2) {
        ...
        $back = trim($card_parts[1]);

        //database insert
        addBatchCardsF($conn, $deck_id, $front, $back, $form_cardtype, $form_difficulty, $form_tags);
        // debug echos
        // echo "<strong>Added:</strong> Q: $front | A: $back <br>";
        $_SESSION['success'] = "Card added successfully!";
    } else {
        $_SESSION['card_insert_msg'] = "<strong>Skipped:</strong> Line missing a pipe separator: $line <br>";
    }
}
